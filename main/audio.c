...
//8/24/2024 - Doesn't work well - Can't even change between music - just resets and goes to default.....doesnt
//even play the sound effect either
/* 
#include <string.h>
#include "freertos/FreeRTOS.h"
#include "freertos/task.h"
...
#define BUF_SIZE (1024)
#define TRIGGER_CHAR 'V'
#define DEBOUNCE_TIME_MS 200 // 200ms debounce time
...
void uart_task(void *arg)
{
    ...
    uint8_t *data = (uint8_t *)malloc(BUF_SIZE);
    TickType_t last_trigger_time = xTaskGetTickCount();  // For debouncing
    ...
    while (1)
    {
        ...
        if (len)
        {
            ...
            if (current_time - last_trigger_time < pdMS_TO_TICKS(DEBOUNCE_TIME_MS))
            {
            ...
            // Switch case for changing both music and effect files based on number 1-9
            switch (data[0])
            {
                case '1':
                case '2':
                ...
                {
                    int index = data[0] - '0';
                    ESP_LOGI(TAG, "Playing music: /sdcard/music/music%d.wav", index);
                    ESP_LOGI(TAG, "Triggering effect: /sdcard/effect/effect%d.wav", index);
                    ...
                    char music_uri[30];
                    snprintf(music_uri, sizeof(music_uri), "/sdcard/music/music%d.wav", index);
                    ...
                    char effect_uri[30];
                    snprintf(effect_uri, sizeof(effect_uri), "/sdcard/effect/effect%d.wav", index);
...
//LAST ONE WE DID TRYING TO CHANGE MUSIC AND SOUND EFFECTS - WORKED BUT DIDN'T WORK
#include <string.h>
#include "freertos/FreeRTOS.h"
#include "freertos/task.h"
#include "freertos/event_groups.h"
...
// WORKING SIMULTANEOUS MUSIC AND SOUND EFFECTS
/*
#include <string.h>
#include "freertos/FreeRTOS.h"
#include "freertos/task.h"
...
/* WELL IM DUMB.  IT WAS JUST THE WRONG FILE NAME.  UART WORKS!!!!
#include <string.h>
#include "freertos/FreeRTOS.h"
#include "freertos/task.h"
#include "esp_log.h"
...
void play_effect() {
    ...
    audio_element_set_uri(fatfs_stream_reader, "/sdcard/8_bit_sound.wav");
